...
INVOICES_HEADER = [
    ...
    # — Поданные к ГД, но ещё НЕ ОПЛАЧЕННЫЕ затраты (149), owner 2026-09-03
    #   «Да заводить». Величина (pending_cost_total) заведена деплоем pendingcost
    #   03.09 и уже входит в bg_cost/прибыль BL, а формула BG «=BR+BS+BT+BU» её
    #   не видела — лист расходился с расчётом бота ровно на неё. Колонка держит
    #   поданное ОТДЕЛЬНО, чтобы «факт»-колонки BR/BS/BT/BU остались честным
    #   фактом ОПЛАЧЕННОГО.
    #   ⚠️ Строго ПОСЛЕДНЯЯ: индексы cells[...] числовые, вставка в середину
    #   сдвинула бы весь модуль.
    "Ожидает оплаты",         # 149 (ET) — pending_cost_total (поданные, не оплаченные)
]
...
class GoogleSheetsService:
    ...
    def _invoice_cells(
        ...
        *,
        ...
        # 2026-09-18 (owner: «ошибку по подмешиванию данных обязательно починить»).
        # В БАЗУ расчёта идёт ТОЛЬКО estimated_materials — зеркало «Импорт ОП» M.
        # Стекло/профиль — поля БОТА, в ОП их нет, и складывать их с ОП M значило
        # считать материалы ДВАЖДЫ: у 26820-1НПН стекло 73 334 в точности равно
        # ОП M 73 334, у 26721-1НПН 115 200 против 124 000.
        # Это ТОТ ЖЕ дефект, что 28.07 починили для ПОКАЗА колонки Q, — тогда
        # расчёты намеренно не трогали («materials_total ниже НЕ трогаем»), и
        # двойной счёт остался жить в V/W/прибыли.
        # ✅ Доказано совпадением с офисом по ОБЕИМ налоговым колонкам (замер 18.09):
        #   26721-1НПН  V −10 856 → 9 918 = ОП НДС 9 918 ; W −16 086 → 7 520 = ОП S 7 520
        #   26820-1НПН  V  10 519 → 23 743 = ОП НДС 23 743 ; W −4 515 → 10 512 = ОП S 10 512
        # Сходимость W с ОП S по всем счетам: 22/24 → 24/24.
        # ⚠️ Отрицательный НДС у 26721-1НПН был заведомо невозможен при плюсовой сумме —
        # это и был видимый симптом двойного счёта.
        # ⚠️ materials_total НЕ удалена: её печатает блок Q/R/S/T выше по своим правилам.
        _plan_mat = est_mat_legacy
        _est_total = _plan_mat + est_inst + est_load + est_log
        if is_credit:
            _nds = 0
            _profit_tax = 0
        else:
            _nds = (_amount * 22 / 122) - (_plan_mat * 22 / 122) if _amount else 0
        ...
        # ET «Ожидает оплаты» (149) — поданные к ГД, но НЕ оплаченные затраты
        # (owner 2026-09-03 «Да заводить»). Величина уже посчитана в cost-card
        # деплоем pendingcost 03.09 и уже сидит в bg_cost/прибыли BL, а формула BG
        # её не видела — строка листа расходилась с расчётом бота ровно на неё.
        # 🔴 Пишется ЗДЕСЬ, рядом с BG, а НЕ в блоке `if _c:` ниже — две причины:
        #   1) `if _c:` ≠ гейт. Cost-card грузится и при закрытом _fact_visible
        #      (integration_hub грузит её КАЖДОМУ материнскому счёту, без статус-гейта),
        #      а _fmt_amount(0) возвращает строку "0" — колонка печатала бы «0» ровно
        #      там, где BG намеренно пуста (на 08.09 это 26225-1КИА и КВ 12), нарушая
        #      соглашение соседнего блока, который гасит BL/BM/BN/BO явной else-веткой.
        #   2) BG пишется КАЖДЫЙ синк безусловно. Обновляйся ET только при наличии
        #      cost-card — живая формула Google получала бы УСТАРЕВШИЙ операнд: полных
        #      экспортов без cost-card два (daily_sync, handlers/common), _need_cost не
        #      пускает шесть статусов, а отмена задачи на оплату зовёт sync_task без
        #      sync_invoice_row. Пустая ячейка в +-формуле = 0, поэтому явное гашение
        #      безопасно и снимает залипание.
        # ⚠️ У кредитных pending всегда 0 (исключены в db.py до отдельного решения
        # owner), поэтому ET у них пуста — это ожидаемо, а не дефект.
        # + AM «Выпл.Агент» (38) — owner 2026-09-08 дословно: «Агентское АМ — тоже
        # участвует в расчете». Агентское входит в bg_cost с 17.06 (db.py:3186), и
        # определение BG в коде его содержит («BR+BS+BT+BU + агентское АМ»,
        # db.py:3082) — а формула Google ему никогда не соответствовала.
        # ⚠️ ЗНАЕМОЕ ОГРАНИЧЕНИЕ, замер 08.09: AM — ЗЕРКАЛО ОП AE и печатает
        # agent_payout_op (:905), тогда как расчёт берёт `agent_payout_op or
        # agent_fee` (db.py:3032). Там, где агентское живёт ТОЛЬКО в agent_fee,
        # формула недосчитает: на 08.09 это 26225-1КИА (30 000 ₽) — у него гейт
        # ещё закрыт и формулы нет, но после «Счёт ОК» разрыв проявится.
        # ⛔ Лечить подстановкой agent_fee в AM НЕЛЬЗЯ — исказит зеркало ОП
        # ([[feedback_op_mirror_no_mixing]]); решение за owner.
        _pending_et = float(_c.get("pending_cost_total") or 0)
        if not _fact_visible:
            cells[58] = ""
            cells[149] = ""
        else:
            cells[58] = f"=BR{row}+BS{row}+BT{row}+BU{row}+AM{row}+ET{row}"
            cells[149] = self._fmt_amount(_pending_et) if _pending_et > 0 else ""
        ...
        # AL «Запрос НОВЫЙ» (37) и AN «Выпл.МенЗП» (39) — заказ owner'а 13.08 дословно:
        # «если bm значения <0 то al=aj-er», «в an данные вносятся после выплаты (либо
        # зачета за счет аванса), сумма: an=aj-er».
        # Смысл: к выплате причитается выплата МИНУС уже зачтённая из авансового
        # кошелька переплата.
        #   AJ  (35) = manager_zp_blank «по бланку» — ⚠️ буква ЛИСТА Invoices, а НЕ
        #        «Импорт ОП» AJ (zp_manager_payout). Ответ owner'а 13.08 на прямой
        #        вопрос; соглашение то же, что в [[reference_invoices_sheet_columns]]:
        #        «я называю названия столбцов бд бота и листа invoices».
        #   ER  (147) = zp_hold_advanced    — переплата, перенесённая в аванс;
        #   ES  (148) = zp_hold_advanced_at — дата согласия на вычет (уже была, :1514).
        #   AM  (38) = agent_payout_op — агентские, ОТДЕЛЬНЫЙ payout
        #        ([[feedback_am_agency_separate_payout]]): к AL ПРИБАВЛЯЕТСЯ, из ЗП не
        #        вычитается. Owner 13.08: «к колонке al суммируется am — это правильно».
        # 🔑 Данные подтверждают связку: у 26119-1КВ бланк 2 850 + AM 29 700 = 32 550 —
        # ровно ОП-выплата. Формула восстанавливает итог из двух слагаемых, поэтому
        # прежняя редакция (ОП AJ) на таких счетах молча складывала агентские дважды.
        # Стоит ПОСЛЕ блока BM по той же причине, что и подмена AK: cells[37]/cells[39]
        # заданы в словаре-литерале (~870), а cells[64] считается здесь — в литерале
        # правку затёрло бы.
        # ⚠️ Пишем ЧИСЛОМ, а не формулой «=AJ{row}-ER{row}»: формула ровно в этой
        # колонке (деплой anhold 09.08) уже дала петлю минусов через импорт ОП и была
        # откачена 12.08 — повторять её нельзя.
        _aj_blank = float(invoice.get("manager_zp_blank") or 0)
        _er_hold = float(invoice.get("zp_hold_advanced") or 0)
        _am_agent = float(invoice.get("agent_payout_op") or 0)
        _zp_net = _aj_blank - _er_hold
        if _bm_val < 0 and _aj_blank:
            # Гейт — тот же _bm_val, что у AK: «-0» от округления перерасчётом не
            # считается (26423-1КИА). Ветка «BM >= 0» НЕ трогается — там AL остаётся
            # зеркалом ОП AI / legacy-текстом, как было.
            # ⚠️ Пустой бланк ветку НЕ включает: иначе счёт без данных по ЗП получал бы
            # literal «0» вместо прежнего зеркала ОП (26525-1КВ). Это НЕ ослабление
            # правила owner'а, а отказ печатать ноль там, где считать не из чего.
            cells[37] = self._fmt_amount(_zp_net + _am_agent)
        if _an_payout_done:
            # Гейт события (выплата ЛИБО зачёт) введён деплоем anpaid 12.08 и здесь НЕ
            # меняется: zp_manager_payout_date непуста ЛИБО zp_hold_advanced != 0.
            # Меняется только ВЕЛИЧИНА.
            # AM прибавляется и здесь — owner 13.08: «к an прибавлять am». Прежняя
            # редакция (amesgate до 13.08) суммировала агентские ТОЛЬКО к AL и
            # закрепляла это комментарием — указание было понято неверно.
            # 🔑 Данные подтверждают: у 26119-1КВ бланк 2 850 + AM 29 700 = 32 550 —
            # ровно ОП-выплата, то есть фактическая ЗП менеджера включает агентские.
            # Формула у AL и AN одна, различаются только гейты: AL — по BM < 0
            # (сколько причитается), AN — по событию выплаты (сколько выплачено).
            cells[39] = self._fmt_amount(_zp_net + _am_agent)
        ...
        # owner 13.08: триггер простановки N «Дата Факт» — статус «Счет End» в AZ,
        # ЛЮБОЕ его срабатывание (монтажники + прочие ветки). Здесь только СНИМОК
        # уже вычисленной метки в сам словарь счёта — ни в БД, ни на лист отсюда
        # ничего не пишется (`feedback_card_display_only_no_data_writes`).
        # Записывает дату вызывающий: integration_hub (одиночный ряд) и
        # sheets_sync (ночной полный экспорт). Снимок стоит ПОСЛЕ BS-override,
        # иначе ветка «BS > 0» в метку не попадёт.
        invoice["_az_label"] = cells[51]
        ...
        # ⚠️ ES берёт zp_hold_confirmed_at, а НЕ zp_hold_advanced_at (owner 13.08:
        # «es носить данные только после того, как менеджер закроет переплату из
        # авансового кошелька за счёт зп-менеджер»). Каналов переноса два, и дату
        # согласия пишет только ОДИН: ручной create_recalc_advance_topup («✅ С
        # перерасчётом согласен»). Авто-свип sweep_manager_overpay_to_advance
        # переносит переплату БЕЗ участия менеджера — по нему ES обязана молчать.
        # 🔑 Проверено на боевых 13.08: у всех 5 счетов с непустой ES задач
        # recalc_confirm НОЛЬ, а у 26114-1КВ/КВ 2/КВ 3 один общий таймстамп
        # 2026-06-24T09:15:29 = пакетный свип. То есть колонка показывала даты
        # согласий, которых не было. zp_hold_advanced_at остаётся техническим
        # полем (трекер последнего переноса) и НЕ удаляется.
        # ER (147) не трогаем: сумма переплаты верна независимо от канала.
        _hold_adv = float(invoice.get("zp_hold_advanced") or 0)
        cells[147] = self._fmt_amount(_hold_adv) if _hold_adv > 0 else ""
        cells[148] = self._fmt_sheet_date(invoice.get("zp_hold_confirmed_at"))
