    ...
    def _invoice_cells(
        ...
        *,
        ...
        if _an_payout_done:
            ...
            # Меняется только ВЕЛИЧИНА. AM сюда НЕ прибавляется — owner суммировал его
            # ровно к AL; AN остаётся «сколько ЗП по бланку причитается за вычетом
            # зачтённой переплаты».
            cells[39] = self._fmt_amount(_zp_net)
