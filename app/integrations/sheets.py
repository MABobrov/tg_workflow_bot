...
class GoogleSheetsService:
    ...
    def _invoice_cells(
        ...
        *,
        ...
        # кошелька переплата.
        #   AJ = «Импорт ОП» AJ «Выплата. Мен. ЗП» = zp_manager_payout (парс-карта :2277)
        #        — ⚠️ ИМЕННО ОП-буква, а НЕ Invoices AJ (35, manager_zp_blank «по
        #        бланку»): источник у AN НЕ меняется, правка только вычитает ER.
        #        Ошибка разбора 13.08 (взял Invoices AJ) двигала 26119-1КВ на −29 700
        #        и 26522-1КВ на +6 420 — счета, где переплаты нет вовсе.
        ...
        _aj_payout = float(invoice.get("zp_manager_payout") or 0)
        _er_hold = float(invoice.get("zp_hold_advanced") or 0)
        _zp_net = _aj_payout - _er_hold
        if _bm_val < 0:
            ...
            cells[37] = self._fmt_amount(_zp_net)
        if _an_payout_done:
            ...
            # Меняется только ВЕЛИЧИНА — вместо зеркала ОП AJ печатается AJ − ER.
