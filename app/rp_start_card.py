...
# ⚠️ «Г» ЗАГЛАВНАЯ тоже (owner 15.08): re регистрозависим, и адрес «Г. Москва ул
# Ленинградский проспект…» (КВ 13) мимо этой маски проходил целиком — город не
# отрезался ни здесь, ни в _street, и в карточку ехало «Г. Москва ул Л».
_CITY_RE = re.compile(r'^\s*[Гг]\.?\s*([А-ЯЁ][А-Яа-яЁё-]*)')
...
def _street(addr, width):
    a = (addr or "").strip()
    # «Г» заглавная — наравне со строчной (owner 15.08), см. _CITY_RE выше.
    a = re.sub(r'^[Гг]\.?\s*[А-Яа-яЁё-]+\s*,?\s*', '', a)
    # Ведущий тип улицы БЕЗ точки («ул Ленинградский проспект…», КВ 13) списком
    # ниже не ловится: там замена по ВСЕЙ строке, и голое «ул» испортило бы
    # «Тульская» → «Т ьская». Здесь якорь ^ и обязательный пробел после токена,
    # поэтому срезается только ведущее слово (owner 15.08).
    a = re.sub(r'^(?:ул|просп|пр-кт|наб|бул|пер|ш)\.?\s+', '', a, flags=re.IGNORECASE)
