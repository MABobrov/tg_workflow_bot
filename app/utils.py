...
def card_num(v: Any) -> str:
    ...
    Перенесено 01.09 из handlers/manager_new.py (был `_card_num`) вместе с общим
    билдером финблока: формат числа во втором месте снова развёл бы две витрины ГД.
    """
    try:
...
def card_pct(v: Any) -> str:
    """Процент для карточки: одна цифра после запятой, минус — U+2212.

    Формат взят у канонической карточки план/факт (format_plan_fact_card,
...
def invoice_end_fin_rows(inv: dict[str, Any], pf: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    параметр width НЕ применяется вовсе (см. format_card_section ниже) — строка вида
    ...
    ⚠️ Состав (24.08): НДС, рент-ть расч/факт, ЗП РП 10%, доля ГД, ЗП менеджера
    из AJ. Это вторая половина требования owner 22.08 — «не хватает важной
    информации» — и его же «данные поступают неправильно»; состав и источники
    взяты у канонической карточки format_plan_fact_card и у письменного
    эталона [[feedback_card_calc_sources]], а не придуманы здесь.

    🔑 ЕДИНЫЙ источник состава для ОБОИХ входов ГД (сведено 01.09, когда
    деплой детектора зеркала ОП 27.08 разблокировал utils.py). Пуш-карточка
    менеджера и самостоятельное закрытие ГД берут строки отсюда напрямую
    (manager_new._build_invoice_end_cards), карточка задачи «Счёт End» — через
    обёртку format_invoice_end_financials ниже (tasks.py:364). До слияния
    состав жил в двух копиях, и вход через список задач показывал ГД ДРУГИЕ
    числа: без НДС и с ЗП менеджера из расчётного сплита вместо AJ.

    ⚠️ Только для ГД/ТД — содержит прибыль, себестоимость и распределение ЗП,
    скрытые от РП (см. format_plan_fact_card role='rp'). Вызывающий код обязан
    гейтить роль: manager_new кладёт строки только в карточку ГД (в карточке
    РП их нет), tasks.py проверяет {Role.GD, Role.TD}.

    ⛔ Значения обязаны оставаться БЕЗ html-спецсимволов: utils-путь отдаёт их
    в format_card_section КАК ЕСТЬ, а manager_new экранирует весь блок целиком —
    добавишь сюда свободный текст (имя клиента, адрес, комментарий), и один
    путь покажет «&quot;», а другой сломает разметку. Сейчас здесь только числа,
    проценты и зашитые литералы.
    ...
    # уже реализованное в канонической карточке (format_plan_fact_card,
    ...
    rows: list[tuple[str, str]] = [("Себест-ть расч", card_num(est_total))]
    if not inv.get("is_credit") and abs(float(net_vat or 0)) > 0.5:
        rows.append(("НДС расч", card_num(net_vat)))
    rows.append(("Прибыль расч", card_num(est_profit)))
    rows.append(("Рент-ть расч", card_pct(est_pct)))
    rows.append(("Себест-ть факт", card_num(fact_total) if has_fact else "—"))
    rows.append(("Прибыль факт", card_num(fact_profit) if has_fact else "—"))
    rows.append(("Рент-ть факт", card_pct(fact_pct) if has_fact else "—"))
    ...
    if est_profit and float(est_profit) > 0:
        ...
        rows.append(("ЗП РП 10%", card_num(rp_zp)))
        ...
        # карточка уже читает AJ (format_plan_fact_card). Карточка
        ...
        rows.append(("ЗП менеджера", card_num(manager_zp_net_payout(inv))))
        rows.append(("Доля ГД", card_num(gd_profit)))
...
def format_invoice_end_financials(inv: dict[str, Any], pf: dict[str, Any]) -> str:
    """Финблок «Счёт End» одной <pre>-секцией, display-only (PART B, ТЗ 19.06).

    Обёртка над invoice_end_fin_rows: тот же состав, что видит ГД на
    пуш-карточке и на экране самостоятельного закрытия, но завёрнутый в свою
    секцию — её приклеивает к карточке задачи handlers/tasks.py (второй вход
    ГД — «открыть задачу из списка»).

    Числа справа, ₽ в ячейках нет — эталон карточек
    ([[feedback_card_telegram_pre_alignment]]). Поэтому НЕ compact: в ветке compact
    параметр width не применяется вовсе и строка выходит «Метка: значение» без
    колонки, а card_num убирает ₽ именно РАДИ правого выравнивания — вместе это
    дало бы худшее из двух: ни валюты, ни колонки. Прежняя редакция стояла
    на compact=True с декоративным width=44 и печатала ₽ внутри значений.

    Ширина 34 — та же, что у объединённого блока ГД в manager_new: один и тот же
    счёт обязан выглядеть одинаково в обеих витринах ГД (приём принят owner
    23.08 для карточки «Счёт снят автором»). 44 не берём: owner 22.08
    жаловался ровно на переносы строк.

    ⚠️ Известная мелочь: format_card_section считает отступ len(), а не vw(),
    и единственная строка с emoji («Соотношение») выходит на одну колонку
    шире прочих. Общий хелпер под vw() НЕ переписываем — это сдвинуло бы
    КАЖДУЮ карточку бота (та же развилка, что в addrcard 16.08).

    ⚠️ Только для ГД/ТД — прибыль, себестоимость и распределение ЗП скрыты от РП
    (см. format_plan_fact_card role='rp'). Вызывающий код обязан гейтить роль;
    tasks.py делает это через {Role.GD, Role.TD} & set(parse_roles(viewer_role)).

    Возвращает "" если расчётные данные не заполнены (нечего показывать).
    """
    rows = invoice_end_fin_rows(inv, pf)
    if not rows:
        return ""
    return format_card_section("📊", "Финансы (справочно)", rows, width=34)
