...
async def tinting_done_attach(
    message: Message,
    state: FSMContext,
    storage: MinioStorage | None = None,
) -> None:
    uid = message.from_user.id if message.from_user else "anon"
    att, count = await collect_attachment(message, state, storage, prefix=f"tinter/{uid}")
    if att is None:
...
async def tinting_done_finalize(
    ...
    role_now, isolated_role = resolve_menu_scope(u.id, user_now.role if user_now else Role.TINTER)
