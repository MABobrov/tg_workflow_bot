...
async def _build_invoice_end_cards(
    ...
    # Карточка РП (`msg`) выше НЕ ТРОГАЕТСЯ: её вывод обязан остаться байт в
    # байт, это проверяется стендом.
    # ⚠️ initiator приходит УЖЕ экранированным: get_initiator_label (utils.py)
    # прогоняет имя и username через html.quote. Блок ниже экранируется целиком,
    # поэтому здесь нужно вернуть сырой вид — иначе имя с «&» уехало бы в чат как
    # «&amp;amp;», а ширина считалась бы по экранированному (тот самый дефект,
    # который эта правка чинит для «Клиента»). На 24.08 латентно: спецсимволов
    # нет ни у одного из 8 пользователей, вывод не меняется — проверено стендом.
    # ⛔ Для карточки РП initiator берётся ЭКРАНИРОВАННЫМ, как и был: там
    # format_card_section сам не экранирует.
    gd_groups: list[list[str]] = [[
        _gd_row("От", _html.unescape(initiator)),
...
async def invoice_end_gd_final(
    ...
    if decision in ("end", "force"):
        ...
        # owner 22.09: «Счёт End при долге — это ошибка». Долг блокирует ОБА
        # решения, включая «⚠️ Закрыть с задачами». Форс закрывает счёт вопреки
        # невыполненным условиям, заводя менеджеру задачи на устранение, — но
        # долг «устранить задачей» нельзя: деньги просто не получены. Тот же
        # порядок, что у кредита с невыплаченной ЗП монтаж выше.
        # ⚠️ Условие берём у check_close_conditions (db.py:6154) — «долг<=0 ИЛИ
        # подтверждение ГД», — чтобы законный override ГД «оплачен 100%» при
        # отстающем листе продолжал работать.
        if not conditions.get("no_debts"):
            _debt_s = f"{float(inv.get('outstanding_debt') or 0):,.0f}".replace(",", " ")
            await cb.message.answer(  # type: ignore[union-attr]
                f"⛔️ Счёт №{inv['invoice_number']} — не погашен долг {_debt_s} ₽.\n"
                "«Счет End» недоступен, пока долг не закрыт — в том числе через\n"
                "«⚠️ Закрыть с задачами».\n\n"
                "Если оплата уже прошла, а лист отстаёт — подтвердите «оплачен 100%»."
            )
            return
