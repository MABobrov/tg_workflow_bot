...
def test_main_menu_role_matrix_has_only_role_specific_buttons() -> None:
    expected = {
        Role.GD: [
            "📥 Входящие для ГД", "Счета на Оплату", "🏁 Счёт END", "💸 Оплата поставщику",
            "Чат с РП (НПН)", "Бухгалтерия", "Монтажная гр.", "Отд.Продаж",
            "Замеры", "🔍 Поиск счёта", "❌ Отмена", "📂 Ещё",
        ],
        Role.TD: [
            "📥 Входящие для ГД", "Счета на Оплату", "🏁 Счёт END", "💸 Оплата поставщику",
            "Чат с РП (НПН)", "Бухгалтерия", "Монтажная гр.", "Отд.Продаж",
            "Синхронизация данных", "🔍 Поиск счёта", "❌ Отмена", "📂 Ещё",
        ],
        Role.MANAGER: [
            "Задачи / Лид в проект", "Проверить КП / Счет", "Счет в Работу", "Счет End",
            "Замеры", "Бухгалтерия (Док./ЭДО)", "Монтажная гр.", "Чат с РП",
            "Мои Счета", "Еще", "Синхронизация данных", "Отмена",
        ],
        Role.MANAGER_KV: [
            "Задачи / Лид в проект", "Проверить КП / Счет", "Счет в Работу", "Счет End",
            "Замеры", "Бухгалтерия (Док./ЭДО)", "Монтажная гр.", "Чат с РП",
        ...
        Role.MANAGER_NPN: [
            "📋 РП", "✅ Менеджер НПН", "Задачи / Лид в проект", "Проверить КП / Счет",
            "Счет в Работу", "Счет End", "Замеры", "Бухгалтерия (Док./ЭДО)",
            "Монтажная гр.", "Чат с РП", "Мои Счета", "Еще",
            "Синхронизация данных", "Отмена",
        ],
        Role.RP: [
            "✅ РП", "👤 Менеджер НПН", "Проверка КП / Выставление Счета", "Чат с ГД",
            "Счета в Работе", "Менеджер 1 (КВ)", "Счета на оплату", "Менеджер 2 (КИА)",
            "Бухгалтерия (УПД)", "Монтажная гр.", "Счет закрыт", "Лид на расчет",
            "❌ Отмена", "📂 Еще",
        ],
        Role.ACCOUNTING: [
            "📥 Входящие задачи", "📊 Счета в работе", "🔍 Поиск счёта", "🏁 Закрытые Счета",
            "🔄 Синхронизация данных", "🚨 Срочно ГД", "📋 Все задачи", "❌ Отмена", "📚 Справка",
        ],
        Role.INSTALLER: [
            "📥 Входящие задачи", "🔨 В Работу", "📦 Заказ материалов", "📦 Заказ доп.материалов",
            "✅ Счет ок", "📐 Размеры ОК", "📌 Мои объекты", "📝 Отчёт за день",
            "💰 Запрос ЗП", "📩 Не срочно ГД", "🚨 Срочно ГД", "🔄 Синхронизация данных",
            "📋 Все задачи", "❌ Отмена", "📚 Справка",
        ],
        Role.ZAMERY: [
            "📋 Заявка на замер", "📋 Мои замеры", "📅 График замеров", "💰 Оплата замеров",
            "🚨 Срочно ГД", "🔄 Синхронизация данных", "📋 Все задачи", "📚 Справка",
            "⬅️ Назад в главное меню",
        ],
        Role.DRIVER: [
            "📥 Входящие задачи", "✅ Доставка выполнена", "📂 Ещё действия",
            "📚 Справка", "🔄 Обновить меню", "❌ Отмена",
        ],
        Role.LOADER: [
            "📥 Входящие задачи", "📂 Ещё действия", "📚 Справка", "🔄 Обновить меню", "❌ Отмена",
        ],
        Role.TINTER: [
            "📥 Входящие задачи", "✅ Тонировка выполнена", "📂 Ещё действия",
            "📚 Справка", "🔄 Обновить меню", "❌ Отмена",
        ],
    }

    for role, labels in expected.items():
        assert _menu_labels(role) == labels
